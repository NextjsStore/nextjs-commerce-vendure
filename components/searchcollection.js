...
export default function Search_Collection(props) {
    const classes = useStyles();
    const { product } = props;
    console.warn('search-collection', product);
    let imageURl = '';
    return (
        <>
            ...
            <Grid item lg={1} md={1} sm={1}>
                <Box className={classes.box_product}>
                    ...
                    <Link href={`/product/${product?.slug}`}>
                        <a>
                            <img className={classes.productImg} src={product.productAsset.preview} alt="" />
                        </a>
                    </Link>
